...
--
PALETTE.param1 					= {}
PALETTE.param1.BOXSIZE 		  	= 32						  -- Taille boite de couleur
PALETTE.param1.BOXSIZESELECT   	= PALETTE.param1.BOXSIZE * .2 -- Taille boite selectionée
PALETTE.param1.BOXOFFSETSELECT 	= {}			 			  -- Offset boite selectionée
PALETTE.param1.LINECOLLIDED	  	= { 1,.8, 1,1}				  -- Color  line collision
PALETTE.SELECTED		  		= {.6,.4,.7,1}				  -- Color  boite selectionée
...
--
	...
	local posX = PALETTE.param1.BOXOFFSETSELECT.W + PALETTE.Position.x +
				 ((PALETTE.COLORINDEX - 1) * PALETTE.param1.BOXSIZE)	
	--
...
--
	...
	--
		local posX = PALETTE.Position.x + ((idx - 1) * PALETTE.param1.BOXSIZE)
		DrawBox(posX, PALETTE.Position.y, PALETTE.param1.BOXSIZE, PALETTE.param1.BOXSIZE, 
...
--
	local posX = ((idxColor - 1) * PALETTE.param1.BOXSIZE) + PALETTE.Position.x
	local posY = PALETTE.Position.y
	--
	local AABB = CheckCollision(LM.getX(), LM.getY(), 1, 1, 
								posX, posY, 
								PALETTE.param1.BOXSIZE, PALETTE.param1.BOXSIZE)
	--
